...
## Domain: NIP-DC (Nostr Webxdc)

This app is a webxdc explorer. Spec: <https://gitlab.com/soapbox-pub/dittoroma/-/blob/main/NOSTR_WEBXDC.md>

- **webxdc app events** ("本体"): any event with an `imeta` tag whose `m` is `application/x-webxdc`, or a kind `1063` NIP-94 file metadata event (`#m` is relay-filterable). The `webxdc` imeta property / tag is the coordination identifier (`i`).
- **Kind 4932**: webxdc state updates (`sendUpdate`), `i` tag = the identifier — filter with `#i`.
- **Kind 20932**: ephemeral realtime data — never stored by relays, don't query history.

All parsing lives in `src/lib/webxdc.ts` (`getWebxdcAttachments`, `isWebxdcAppEvent`, `getWebxdcId`, `getWebxdcName`, `parseWebxdcUpdate`, `sortWebxdcUpdates`). Reuse it — don't re-implement imeta/tag parsing. The shared debug viewer is `src/components/webxdc/EventJsonDialog.tsx`; grid/list switching is `src/components/webxdc/ViewToggle.tsx` (persist via `useLocalStorage`).

Relays can't filter on `imeta`/`webxdc` (multi-letter tags) — discover apps via `{ kinds: [1063], '#m': ['application/x-webxdc'] }` plus NIP-50 `search` (`.xdc`) on supporting relays, then validate client-side.
